...
const RecentPets = () => {
  ...
  const {pets, backendUrl}= useContext(AppContext)
  return (
    <div className="flex flex-col items-center gap-4 my-16 text-gray-900 text-center">
      ...
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 pt-5 px-3 sm:px-0">
        {pets.slice(0, 10).map((item, index) => {
          ...
          const imageSrc = item.image && (item.image.startsWith('http') ? item.image : `${backendUrl}/${item.image.replace(/^\/+/, '')}`)
          return (
          <div onClick={()=>{ if(petId){ navigate(`/appointment/${petId}`); scrollTo(0,0)} }}
